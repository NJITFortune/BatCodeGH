function [im, dat] = nfwt(dire, lims)
% [im dat] = nfwt(directory, limits) 
% directory is the directory - e.g. '/Volumes/BatData/fem3'
% limits is the range of frames, e.g. [1000 3000]
% limits is optional... if you don't include it, the code will run all
% frames.
% im is the image data   
% dat is the tracked data
% This code SUCKS.  It is a rotting heap of stinky poop. It should be
% re-written from scratch.

%% Defaults and setup

cd(dire); flist = dir('*.jpg'); cd ..;

     xbuff = 40; % This is the X-window width for looking for Y position of wingtips. 40
     ybuff = 20; % This is the X-window width for looking for Y position of wingtips. 20

    % If the user does not specify the limits, do the whole list of jpegs.
    if nargin == 1; % Did not include lims
...
%% First click
    ...
    cd(dire); firstimage = rgb2gray(imread(flist(1).name)); cd ..;
...
%% Find X and Y of outer edges of wings - in short, Tracking

k = qNumFrames;

while k ~= 0; % k is the number of frames - we cycle down to zero as we track. This is our main loop.
    
    skipanal = 0; % This indicates when we lose the track and make the user do the tracking for us.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
% EMPTY hindx indicates that the moth has stopped moving. We need to track the wings by hand. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    

    if isempty(hindx{k});  

        JumpNum =15; % This is the numer of frames between clicks.
        
        for i = -JumpNum*10:JumpNum:JumpNum*5;
            
        figure(10); subplot(4,4,(i/JumpNum)+11); 
            imshow(bw(:,:,k+i)); 
            hold on;
            text(10,90, num2str((i/JumpNum)+11), 'FontSize', 24, 'FontWeight', 'bold', 'Color', 'w'); 
            if i == 0;
                text(10,240, 'TrackFail', 'FontSize', 14, 'FontWeight', 'bold', 'Color', 'w'); 
            end;
            
%         figure(11); subplot(4,4,(i/10)+11);
%             imshow(g(:,:,k+i)); 
%             hold on;
%             text(40,40, num2str(k+i), 'Color', 'g'); 
%             plot(xcenter, ycenter, 'g*');
            
        end; % Catch bad frames
        
            firstgood = (k-(JumpNum*10)) + (JumpNum * (input('Number of the last good frame before the outage? ') - 1));
            lastgood = (k-(JumpNum*10)) + (JumpNum * (input('Number of the frist good frame after the outage? ') - 1));
  
              figure(3); 
              imshow(g(:,:,lastgood)); 
              hold on; plot(xcenter, ycenter, 'm*'); 
              plot(minX(lastgood), RightY(lastgood), 'r*');
              plot(maxX(lastgood), LeftY(lastgood), 'g*');
              pause(0.5);

              lastfill = lastgood;

              for jj = lastgood-JumpNum:-JumpNum:firstgood;
                  figure(3); 
                  imshow(g(:,:,jj)); 
                  hold on; plot(xcenter, ycenter, 'm*');
                  [fixX(1), fixY(1)] = ginput(1);
                  plot(fixX(1), fixY(1), 'r*');
                  [fixX(2), fixY(2)] = ginput(1);
                  plot(fixX(2), fixY(2), 'g*');
                  pause(0.25);

                  % Right
                  %minX(jj) = fixX(1); RightY(jj) = fixY(1);
                  minX(jj:lastfill) = linspace(fixX(1), minX(lastfill), JumpNum+1);
                  RightY(jj:lastfill) = linspace(fixY(1), RightY(lastfill), JumpNum+1);
                  % Left
                  %maxX(jj) = fixX(2); LeftY(jj) = fixY(2);
                  maxX(jj:lastfill) = linspace(fixX(2), maxX(lastfill), JumpNum+1);
                  LeftY(jj:lastfill) = linspace(fixY(2), LeftY(lastfill), JumpNum+1);

                  lastfill = lastfill-JumpNum;

              end;
              
 %             figure(7); plot(minX, RightY, 'r', maxX, LeftY, 'g');
              skipanal = 1;
              k = firstgood;
    end;
...
%% Moth's Right wing (our Left side) [minX(k) maxX(k)+xbuff], [LefttBottomIdx(k) LefttTopIdx(k)]
     ...
     % Detect the vertical location of the box
        ...
        % Catch terrible tracking - when the wingtip is on the wrong side
        % of the animal.
    if minX(k) > xcenter;
        figure(1); clf; imshow(g(:,:,k)); hold on; plot(minX(k), RightY(k), 'm*');
        [minX(k), RightY(k)] = ginput(1); plot(minX(k), RightY(k), 'r*');
            pause(0.1);
    end;
    
    if maxX(k) < xcenter;
        figure(2); clf; imshow(g(:,:,k)); hold on; plot(maxX(k), LeftY(k), 'c*');
        [maxX(k), LeftY(k)] = ginput(1); plot(maxX(k), LeftY(k), 'g*');
            pause(0.1);
    end;
