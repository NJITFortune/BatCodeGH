function im = newfrontwingtrack(lims)
...
%    thresh = 0.4; % Threshold level for grayscale. 0 is black, 1 is white.
%    boxlim = [40 80]; % Size in pixels of our search box [x y] [80 160] originally
%    cnt = 5; % The number of white pixels to qualify - 5 seems to work. Higher will reject more noise and lower is more sensitive.
    ...
    im(lims(2)).orig = zeros(481,640); % This is the biggest memory hog - the stack of images.

    
numframes = lims(2) - lims(1);
for k = numframes:-1:1; g(:, :, k) = rgb2gray(imread(flist(k).name)); end;
for k = numframes-1:-1:1; d(:, :, k) = imabsdiff(g(:, :, k), g(:, :, k+1)); end;    
    thresh = 0.04;
    bw = (d >= thresh * 255); % THRESHOLD IS CRITICAL
    bw2 = bwareaopen(bw, 20, 8);
    
for k = numframes-1:-1:1;
    for j=1:640; hsum(k,j) = sum(bw2(:,j,k)); end;
    for j=1:481; vsum(k,j) = sum(bw2(j,:,k)); end;
end;
...
for k=1:numframes-1;
    pause(0.05);
    figure(1); clf;
    subplot(121); imshow(bw2(:,:,k));
    subplot(222); plot(hsum(k,:));
    subplot(224); plot(vsum(k,:)); view(90,90);
end;
...
%        mybm(:,:,3) = double(ones(481,640));                   
%        mybm(:,:,2) = double(zeros(481,640));                  
% 
% 
